...
def k_means_pp_init(X, K,num_feat=384,num_models=10):
    ...
    for k in range(1, K):
        ...
        next_mean_idx = torch.matmul(return_norm(X[:,  num_feat:]).unsqueeze(0), return_norm(torch.stack(means[:])[:, :,  num_feat:]).permute(0, 2, 1)).sum(0).argmin(0)
...
def crf_seg(crf_net, args, first_anno, img_lab_resize_torch, ind, pred_crf_1):
    ...
    if ind == 0:
        ...
        for epoch in range(range_to_send):
            ...
            image_to_send = (image+1.0*torch.randn(image.shape).cuda()).clip(0,255)
            unary_to_send = unary
