...
class Bilinear(nn.Module):
    ...
    def forward(self, target_low, guide):
        return F.interpolate(target_low, scale_factor=self.factor, mode='bilinear', align_corners = True)
...
class DJIF(nn.Module):
    ...
    def forward(self, target_low, guide):
        ...
        if target_low.shape[-1] < guide.shape[-1]:
            target_low = F.interpolate(target_low, scale_factor=self.factor, mode='bilinear', align_corners = True)
...
class PacJointUpsample(nn.Module):
    ...
    def forward(self, target_low, guide):
        ...
        for i in range(len(self.up_convts)):
            ...
            if scale != 1:
                guide_cur = F.interpolate(guide_cur, scale_factor=scale, align_corners = True, mode='bilinear')
