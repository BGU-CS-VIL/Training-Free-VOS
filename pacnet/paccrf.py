...
def create_YXRGB(img, yx_scale=None, rgb_scale=None, scales=None,create_position=False,use_preposition=None):
    img = img.view(-1, *img.shape[-3:])
    if scales is not None:
        assert yx_scale == None and rgb_scale == None
        yx_scale = scales[:2]
        rgb_scale = scales[2:]
    if use_preposition is not None:
        mesh =use_preposition
    else:
    ...
    def forward(self, unary, edge_feat, edge_kernel=None, logQ=None):
        ...
        for step in range(self.num_steps):
            ...
            for i in range(n_kernels):
                ...
                if self.blur[i] > 1:
                    msg = F.interpolate(msg, scale_factor=self.blur[i], mode='bilinear', align_corners = True)
